...
function Modal(props) {
  ...
  const { children, show, opacity } = props;

  const toggleModal = async () => {
    if (!props.toggleModal) return;
  ...
  React.useEffect(() => {
    if (lightbox.current) {
      const lightboxRect = lightbox.current.getBoundingClientRect();
      const newOffset = lightboxRect.top > 0 ? lightboxRect.top : 0;

      if (newOffset !== INITIAL_OFFSET && newOffset !== offset) {
        setOffset(newOffset);
      }
    }
  });
